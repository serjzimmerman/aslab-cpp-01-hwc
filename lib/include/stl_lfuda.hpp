...
public:
  ...
  it__ push_front(node_t__ p_node) {
    p_node.m_weight = m_weight; // Guarantee that the weight of node corresponds to the weight of the list.
...
template <typename U, typename K = int> class lfuda_t {
  ...
  void evict_and_replace(const K &p_key, U p_val) {
    assert(m_key_it_map.size()); // Cache should contain some elements or I've made an oopsie somewhere.
...
public:
  explicit lfuda_t(std::size_t p_size) : m_size{p_size}, m_hits{0}, m_age{0}, m_weight_map{}, m_key_it_map{} {
    if (!p_size) {
